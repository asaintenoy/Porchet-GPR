...
TWT_AS=F_extractTWT(where+'_AS/')
VOL_AS=np.genfromtxt(where+'_AS/Volumes_EL.csv',delimiter=',',skip_header=1)

TWT_S=F_extractTWT(where+'_S/')
VOL_S=np.genfromtxt(where+'_S/Volumes_EL.csv',delimiter=',',skip_header=1)
...
ax[0].set_xlabel('Toyau Rempli',fontsize=20)
